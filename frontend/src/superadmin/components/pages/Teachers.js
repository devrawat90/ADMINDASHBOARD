...
const Teachers = () => {
    ...
    const handleSumit = async (e) => {
        ...
        await axios.put("http://localhost:8080/editteachers", editform)
            .then((res) => {
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get("http://localhost:8080/getteachers");
    ...
    const handleDelete = async (itemsToDelete) => {
        ...
        if (itemsToDelete && itemsToDelete.data && itemsToDelete.data.length > 0) {
            ...
            itemsToDelete.data.map(async (row) => {
                ...
                try {
                    const response = await axios.delete(`http://localhost:8080/deleteteachers/${idToDelete}`);
    ...
    const handleSubmit2 = (e) => {
        e.preventDefault();
        try {
            axios.post(`http://localhost:8080/addteachers`, adddata).then((resp) => {
