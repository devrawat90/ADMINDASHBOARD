...
const Leads = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            const updateddata = response.data.data.filter((user) => {
                return user.admin_id === matchadmin
            })
                ...
                customBodyRender: (value, tableMeta, updateValue) => {
                    return (
                        <div className='d-flex justify-content-center'>
                            <RemoveRedEyeIcon className='text-primary' onClick={() => handlemessage(data[tableMeta.rowIndex])} />
    ...
    const handleStatusAndMessage = async (e) => {
        e.preventDefault();
        try {
